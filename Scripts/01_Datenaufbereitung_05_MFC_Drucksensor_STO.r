...
##############################################################
#####                                                    #####
#####    Datenaufbereitung MFC und Keller Drucksensor    #####
#####                                                    #####
##############################################################
##############################################################
...
MFC_dir <- dir(file.path(PfadDaten,"MFC"), full.names = TRUE)
...
MFC_data[,Time_new := parse_date_time3(substr(Date,5,27),tz="Etc/GMT-1")]
...
## Werte mit 0 ergänzen, bei denen MFC abgestellt war
...
MFC_dt2[st> "2021-03-20 03:53" & st< "2021-03-20 04:05",Q_MFCex := 6.0225] # Fluss aber nicht geloggt. Darum ergänzen
MFC <- as.ibts(MFC_dt2)

plot(MFC_1min["20.03.2021 03:52 to 20.03.2021 04:06","Q_MFC"])
#check
plot(MFC1[MK,"Q_MFC"])
lines(MFC_1min[,"Q_MFC"],col="blue")
lines(MFC[,"Q_MFCex"],col="red")


saveRDS(MFC,file.path(PfadRSaves,"STO_MFC.rds"))

#### check, if the released gas fits with the amount of gas inside the cyclinder bundle
# pool to 1 min
MFC_1min <- pool(MFC, granularity = "1mins", st= "19.03.2021")
plot(MFC_1min["19.03.2021 to ","Flow_MFC_lnmin"]) # check if timeperiod is correct
plot(MFC_1min["22.03.2021 to ","Flow_MFC_lnmin"]) # check if timeperiod is correct
abline(v=parse_date_time3("22.03.2021	12:30",tz="Etc/GMT-1"),col="red")
colSums(MFC_1min["19.03.2021 to 22.03.2021 12:30","Flow_MFC_lnmin"]) # select the correct period and sum up the Flow
...
colSums(MFC_1min["22.03.2021 12:30 to ","Flow_MFC_lnmin"]) # select the correct period and sum up the Flow
...
plot(MFC_1min[" to 19.03.2021","Flow_MFC_lnmin"]) # check if timeperiod is correct
colSums(MFC_1min[" to 19.03.2021","Flow_MFC_lnmin"]) # select the correct period and sum up the Flow
# 11456.5 nl --> 11.4 m3. That is fine as we expect around 11 m3

# Conc <- readRDS(file.path(PfadRSaves,"STO_Conc_10min.rds"))

# Conc2 <- merge(Conc,MFC,all=TRUE)
# MFC2 <- merge(MFC,Conc,all=TRUE)

# plot(MFC2[,c("GF16","Q_MFC")],ylab="GF16 [ppm]",col="lightgrey",col2="indianred",ylim=c(0,2))
# lines(Conc[MK,"GF17"],col="lightgrey")
# lines(Conc[MK,"GF18"],col="lightgrey")
# lines(Conc[MK,"GF25"],col="lightgrey")
# lines(Conc[MK,"GF26"],col="lightgrey")
...
CC_dir <- dir(file.path(PfadDaten,"Drucksensor"), full.names = TRUE)
...
# CC_Sensor_10min[,"T_sum"] <- NA_real_
# CC_Sensor_10min[which(CC_Sensor_10min["19.03.2021 08:00 - 22.03.2021 12:30","dT_JT"] != "NA"),"T_sum"] <- cumsum(CC_Sensor_10min[which(CC_Sensor_10min["19.03.2021 08:00 - 22.03.2021 12:30","dT_JT"] != "NA"),"dT_JT"])
...
saveRDS(CC_Sensor,file.path(PfadRSaves,"STO_CC_Sensor_raw.rds"))
saveRDS(CC_Sensor_10min,file.path(PfadRSaves,"STO_CC_Sensor.rds"))



plot(MFC_1min[MK,"Q_MFC"],ylim=c(-10,10))
...
plot(MFC_1min["19.03.2021 to ","Q_MFC"],ylim=c(-10,100))
plot(MFC_1min["22.03.2021 to ","Q_MFC"],ylim=c(-10,10))
...
## ad MFC to CC_Sensor
CC_MFC <- merge(CC_Sensor_1min,MFC_1min[,"Q_MFC"])
...
## Temp plotten
plot(CC_MFC[MK,"Temp_CC"],col="blue",ylim=c(-150,10))
lines(CC_MFC[MK,"Temp_CC_exp"]-273.15,col="red")

plot(CC_MFC[QV3,"Temp_CC"],col="blue",ylim=c(-150,10))
lines(CC_MFC[QV3,"Temp_CC_exp"]-273.15,col="red")
...
plot(MFC_1min["19.03.2021 to ","Q_MFC"],ylim=c(-10,10))
plot(MFC_1min["22.03.2021 to ","Q_MFC"],ylim=c(-10,10))
